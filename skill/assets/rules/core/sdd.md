...
- **Analyze before implementation** — judge reuse-first (existing code checked before new code is planned) and over-engineering (nothing unrequested); run the mechanical passes with `python3 docs/ai/engine.py sdd-lint` (coverage R↔task, dangling per-R-NNN references, unresolved placeholders — quoted tokens are quotation, not findings; converged cases are history and are skipped). Where `python3` is absent the passes run as an explicit checklist — a gap to close, never a licence to skip the gate. Findings are proposals; the human decides.
