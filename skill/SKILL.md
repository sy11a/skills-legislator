...
9. **Foreign AI-layer structures (Info):** list any of `.cursorrules`, `.cursor/`, `AGENTS.md`/`agents.md`, `.github/copilot-instructions.md`, `wiki/`, and ADR/plans directories outside the standard layout (`adrs/`, `doc/adr/`, `.claude/plans/`); `.claude/rules/` is standard layout, never foreign. When a listed file's content is predominantly law-shaped (check 12's test), report it here at **Warning** instead of Info — a foreign config full of law is unloaded law, not mere leftovers.
