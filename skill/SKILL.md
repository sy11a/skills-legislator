...
Placeholder derivation rules (fresh-scaffold mode only, except `{{TODAY_ISO}}`, `{{TODAY_ISO_DATE}}`, `{{CODEBASE_MAP_TABLE}}`, `{{GLOSSARY_TABLE}}`, `{{SANCTIONED_SKILLS_BY_STAGE}}`, and `{{BOUNDARIES}}`, which are mode-independent — legacy migration extracts the rest of these from the existing CLAUDE.md instead, per Step 5 and the `docs/okf/index.md` row above; the mode-independent five are always derived the same way regardless of mode):
...
1. Split the existing CLAUDE.md three ways: law-shaped project rules (carved into `.claude/rules/<topic>.md` per §1), project-specific instance data (kept in CLAUDE.md), and content now covered by an owned rule (removed, replaced by the full `CLAUDE.md.tpl` v2 wiring: the `@docs/ai/rules/...` import block, the `@docs/okf/codebase-map.md` import, the `## Boundaries` section, and the glossary pointer line — see `references/migration.md` §1; migration writes these directly rather than proposing them in Step 7).
...
9. **Foreign AI-layer structures (Info):** list any of `.cursorrules`, `.cursor/`, `AGENTS.md`/`agents.md`, `.github/copilot-instructions.md`, `wiki/`, ADR/plans directories outside the standard layout (`adrs/`, `doc/adr/`, `.claude/plans/`), and parallel knowledge/intake stores other skill ecosystems install — root `CONTEXT.md`, `CONTEXT-MAP.md`, `UBIQUITOUS_LANGUAGE.md`, root `NOTES.md`, `docs/agents/`, `.scratch/`; `.claude/rules/` is standard layout, never foreign. When a listed file's content is predominantly law-shaped (check 12's test), report it here at **Warning** instead of Info — a foreign config full of law is unloaded law, not mere leftovers. Kept paths (manifest `keep` list) are exempt from this check entirely — the keep entry is the user's ruling that the artifact stays.
