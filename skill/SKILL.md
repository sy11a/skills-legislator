...
11. **Project-rules integrity (Warning):** every `.md` under `.claude/rules/` is project law (see `core/project-rules.md`). Flag any statement there that contradicts an owned rule under `docs/ai/rules/` — finding `<path>: contradicts <owned rule path> → align it with the owned rule, or record an explicit exception in the file (conflicts are decision-gate material; never silently pick a side)`. Skip when the directory is absent — but when it is absent while the manifest's `legislatorVersion` is current (check 4 clean), add to the **Info** section exactly `- [project-rules] .claude/rules/: directory absent (git does not track empty directories) → mkdir it so project rules have their home`.
