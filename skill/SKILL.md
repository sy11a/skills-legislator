...
12. **Stray rulebooks (Warning):** an `.md` file whose content is predominantly law-shaped rule or checklist items (imperative, diff-checkable — the same law-shaped test as the constitution-candidates scan) that no session ever loads. Scan every `.md` under `docs/` except `docs/ai/rules/**`, `docs/adr/**`, `docs/journal/**`, `docs/backlog.md`, `docs/superpowers/specs/**`, and `docs/superpowers/plans/**`, plus root-level `.md` files other than `CLAUDE.md`, `README.md`, and `CHANGELOG.md`. A file is exempt when CLAUDE.md or any `.md` under `.claude/rules/` references it (check 7's referenced definition, restricted to those two referrers — the surfaces a session actually loads). Finding: `<path>: rule-like document no session loads → run restructure to consolidate it into .claude/rules/`.
