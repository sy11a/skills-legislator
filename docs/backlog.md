...
**Status: DONE 2026-08-27, edition v23** — shipped as `docs/cases/BL-065-case-shape-lints/` (tier 2, R-651–R-660); benchmark `evals/benchmarks/v23.md` (205/205, idem ×3, mutations clean, floor sonnet). Nine lint families red-first; the dropped clauses (sdd-29, chlog-2, okf-6, adr-7, git-aware family) are named in the spec with reasons. First live catches: BL-060's unclosed marker and the edition's own spec defects.
...
**Status: DONE 2026-08-27, edition v23 (audit slice)** — shipped as `docs/cases/BL-066-audit-job-and-emitter/` (tier 2, R-661–R-669): the engine `audit` job (15 mechanical checks) + the report emitter with the model-findings channel, per BL-049's contract. The audit report is an engine print with pinned model slots; the model performs checks 11/12 and the candidates scan only. Step-7/restructure emitters ride v24 with engine apply/verify, per the composition clarify. Chronicle: 3 law + 2 grader defects surfaced and closed red-first, model class still 0-for-3 all-time (`evals/benchmarks/v23.md`).
...
**Status: DONE — light half 2026-08-26, heavy half 2026-08-27 in edition v23** (C1 encoding, F1 fail-loud, D3 `.gitattributes` scaffold, E1/E2 `tools/proc.py` — the v23 benchmark's ~10 agent invocations were the port's live verification) — shipped as `docs/cases/BL-070-portability-patch-set/` (tier 1, R-701–R-707): A3/B2 Windows-style command heads in both arms; A1 interpreter-resolving launcher in hooks.json (`python3` → `py` → `python`; Windows *execution* verification remains an explicit residual); E5 link scripts ported to Python (`tools/link_skills.py`, `tools/link_opencode_plugin.py` — a failed symlink fails loud with the remedy, never a silent copy; bash originals removed); E6 check_engine chmod test skipped loudly on Windows; E8 dashboard `ps` guarded + `/tmp` via `tempfile.gettempdir()`; F2 node ≥ 22.6 floor assert in the mjs harness. Red-first held where a harness exists (5 + 2 reds shown). **The heavy half rides edition v23, parked loudly, not dropped:** C1 (engine `encoding="utf-8"`) + F1 (`okf-debt` git-absence fail-loud) as engine VERSION riders, D3 (the autocrlf/.gitattributes ruling), E1/E2 (evals-bg process control off bash — verifiable only by the live benchmark run v23 performs anyway). Per ADR-0005's phasing this work is not throwaway: it defines the behavior the future binary arm must reproduce, and its measured scope is what BL-072 is sized against.
...
**Done when:** corpus 100% (fresh, link, restore, upgrade, audit, audit-engine-absent, guards, two-root-engine, case-practice, fleet-obs-module, idem ×3), model floor recorded, `evals/benchmarks/v<N>.md` for the number assigned at merge; BL-080's fleet-obs cases merged first; this repository linked from an instance with `git status` clean and every check green through the two-root engine.
...
## BL-082 — The deterministic substrate becomes .NET (the edition after v25, with BL-077)

**Status: ✅ CONVERGED 2026-09-08 — all 14 tasks owned, T-13's ten residue items and T-14.1 closed, the full benchmark recorded (`evals/benchmarks/v26.md`: corpus 212/212, idempotency zero ×3, mutations 212/212) and converge clean on its second pass (ADR-0010, ADR-0011). **Merged 2026-09-08 — PR #43 into `master` as `1c260b7`, edition v26.** BL-077's plan takes the branch from here. The parking condition is met and the owner is executing.** It was PARKED 2026-08-29 for two stated reasons — to road-test dev-flow on a real case, and to keep ownership of the codebase: the first compiled code in this repository is written under the owner's own flow, not by an agent running the plan unattended. That still holds — the case runs one plan task per session, each closed by an owner review; no session starts a task on its own. Branch `bl/082-dotnet-deterministic-substrate`, case `docs/cases/BL-082-dotnet-deterministic-substrate/` (tier 2, feature, R-8201–R-8217, ADR-0008), flow tracker in the operator's knowledge base. Behavioral: `skill/` law text changes command names, a VERSION bump shared with BL-077, full e2e; plus the first `src/` and `tests/` trees and their CI. **The edition number is assigned at merge, never reserved** (roadmap ruling at the v25 merge): the tool pin `src/Legislator.Cli/Version.props` stayed `0.0.0` until Task 12 read the free number — **26** — and the static check has bound `skill/VERSION` to its major since that commit. Rebased onto `master` 2026-09-07 so the benchmark measures the edition that merges; the two backlog rows this branch filed as BL-088/BL-089 moved to BL-091/BL-092, `master` having minted those keys for the Release-0 pair.
...
**The gap, measured on member #0's machine (2026-08-30):** `tools/link_skills.py` knows one source (`~/.agents/skills`) and one destination (`~/.claude/skills`); its KEEP list has no entry for the dev-flow family. The dev-flow skills (`flow`, `flow-setup`, `orient`, `capture`, `report-bug` from `~/Repository/dev-flow/skills`) and the companion `debrief` (`~/Repository/debrief/skills`) reach `~/.claude/skills` *and* `~/.config/opencode/skills` only through the products' own POSIX scripts (`dev-flow/tools/link-skills.sh`, `debrief/tools/link-skill.sh`) — `sh` + `ln -sfn`, so nothing on Windows; and nothing in the legislator's machine-setup (README "Skill ecosystem setup", `link_skills.py --check`, audit check 14) knows they should exist, so a fresh machine silently comes up without `/flow`. The manual state on this machine is correct (all six resolve in both hosts) — the case is about making that the *default* of legislator initialization, not a hand step.

**The change:** (1) `tools/link_skills.py` grows a *source register* instead of one `SRC`: each keep-list entry names its source pack (`~/.agents/skills` default; `dev-flow` and `debrief` as repo-backed packs located by a per-machine path — via the BL-083 options / `~/.config/legislator` instance config once it exists, `DEVFLOW_ROOT`/`DEBRIEF_ROOT` env until then), and two destinations, `~/.claude/skills` and `~/.config/opencode/skills`, both linked in one run (cross-harness parity, BL-044; `--check` reports drift per destination; `--prune` stays scoped to links into a known source). Symlink policy unchanged: R-703 — fail loud with the Windows Developer-Mode remedy, never copy. (2) The pinned stage-affinity list in `SKILL.md` (`{{SANCTIONED_SKILLS_BY_STAGE}}`) and the `.claude/rules/skills.md` template gain the family so a legislated repo's stage map can sanction them when installed: **orient** — `orient`, `flow-setup`; **implement/any stage** — `flow` (the stage runner), `capture`; **debug** — `report-bug`; **session exit** — `debrief`. Still "never list a skill that is not installed". (3) README "Skill ecosystem setup" documents the dev-flow pack as the third clone-and-link source and the one command that links everything for both harnesses. (4) The eval fixture for check 14 gets a case where a dev-flow skill is sanctioned but absent, proving the Info line names the remedy.

**Ownership boundary:** the products keep owning their skill *content* and their own link scripts (dev-flow ADR-0004 delivery route; `/debrief` is owned by the debrief repo). The legislator only learns *where they live* and links them as part of machine setup — it never vendors them and never edits them. If the BL-072/082 machine-installed binary lands first, the register lives there and `link_skills.py` becomes its Python shim; the entry-level contract (sources × destinations, fail-loud symlinks) is the same either way.

**Done when:** on a fresh machine (Linux, macOS, Windows with Developer Mode) one legislator setup command leaves `/flow`, `/orient`, `/capture`, `/report-bug`, `/flow-setup`, `/debrief` invokable in both Claude Code and opencode; `link_skills.py --check` is clean on member #0; audit check 14 flags a missing dev-flow skill with the remedy; README updated. Behavioral (`skill/SKILL.md` + template text change): VERSION bump, full e2e per `evals/README.md`. Depends on: BL-070 portability patch set (landed); coordinates with BL-083 for where the per-machine paths live.
...
**Options to weigh (the spike picks one, with the owner):** (A) **Composition** — legislation ends with a gated hand-off: "the repository is legislated; bind its sources now? → invoke `/flow-setup <project>`", the skill stays whole and owned by dev-flow, the legislator only sequences it and passes what it already knows (project name, `case home`, detected remote/tracker signals from BL-085) so those windows are pre-filled, never re-asked. (B) **Shared detection, separate writes** — one deterministic detection job (engine, BL-082 substrate) produces the signal table; legislation consumes it for the constitution, `/flow-setup` consumes it for the sources note; each keeps its own writer and file authority (BL-038 matrix: the note is the operator's KB, never a repo artifact). (C) **Absorption** — legislation itself runs the source interview and writes the sources note. Likely rejected: it breaks the abstraction rule of dev-flow (project methods live in the KB by link, ADR-0002) and the ownership boundary of BL-086, and puts an interview with MCP config writes inside a procedure whose contract is "never write outside the target repo" (`SKILL.md` §0) — but state the rejection with reasons rather than assume it.

**What must be true in every option:** nothing is asked twice (the sources note is the memory, `/flow-setup` §1); MCP wiring stays decision-gated on the dev-flow side; the legislator's "never write outside the repo" rule holds — if legislation triggers writes to the KB, that is `/flow-setup` writing under its own law, in its own window; the step is skippable and idempotent on upgrade (a bound project reports "found, bound" and moves on); opencode parity (BL-044) — the hand-off works where `disable-model-invocation: true` means the operator, not the model, types `/flow-setup`.

**What the answer decides:** whether `SKILL.md` gains a final "bind sources" step (mode-independent, gated) or a report line only; whether BL-085's detection table is shared with dev-flow as a contract (a format both repos read); what the legislator passes and in which shape (arguments to `/flow-setup`, or a file it drops for the skill to find); and whether the audit gains a check "legislated but unbound" (Info, environment-relative like check 14).
...
**What:** every forward reference to an edition number in `docs/backlog.md` becomes a reference to the case that carries it. Known offenders: BL-083 ("v26, with BL-078 + BL-076", "VERSION 26 shared"), BL-084 ("v27, with BL-079", "VERSION 27 shared"), BL-080 ("must merge **before v25** ships"), and the roadmap's own numbered steps. A row states its *order* ("the edition after BL-082 + BL-077") and its *behavioral class* ("a VERSION bump, full e2e"); the integer arrives at merge and nowhere earlier.

**Why:** a reserved number is a claim about merge order that the queue is free to break — and it broke once already, silently, leaving five artifacts across two cases asserting a version that another edition had taken. Numbers read as identifiers, so a stale one is not obviously stale; a case name is checkable by eye. The static check `skill/VERSION == major(Version.props)` (BL-082 Task 12) enforces the *pair* at delivery, but nothing enforces a prose claim about a future edition — so the fix is to stop making them.
...
**Why:** the baseline is a verification artifact, and a verification artifact that reports coverage nobody wrote is the BL-069 F1 failure in another dress — a false green a human reads and believes. It bit here at ten rows and would have grown with every ported job's fixtures; T-08 broke the literal in its own fixtures (`"per " + "R-001"`) to keep the register honest, which is a workaround living in test code, not a rule. `tests/Legislator.Parity.Tests/Labels.cs` shows the other half of the question: its `per R-702` is a genuine reference by code that does implement that requirement, and no mechanical rule proposed here would keep it while dropping a fixture's.
...
**Status: delivered 2026-09-08 — draft PR #47 open against `master`, awaiting the operator's review and merge (acceptance record `docs/cases/L-1-law-reads-by-mode/acceptance.md`, benchmark `evals/benchmarks/v27.md`).** The branch `l/1-law-reads-by-mode` carries the delivered work (`cf84e19`..`4dcc541`, 13 commits); the edition question (v27, batch or alone) travels in the PR body as an explicit proposal per DP-1, never a decision of the task. This case's own exit is the narrower source-side half (`spec.md`'s closing paragraph): the law text, the template line, audit check 21 and its fixture land; the kernel half (Architector ADR 0007's own ledger, "and its kernel" — a repository's release-branch convention is checked, its kernel is not) is recorded as a finding against the kernel, carried to the release ledger; the fleet delivery follows the release runbook once the edition is cut. Case `docs/cases/L-1-law-reads-by-mode/`, ADR-0012. Raised by Architector's case BL-008 (`~/Repository/Architector/docs/adr/0007-waterflow-mode.md`, status *proposed* there because the ruling and the edition are this repository's). Behavioral change to `assets/rules/core/pair-development.md`: VERSION bump (proposed, not taken), full benchmark (run — `v27.md`).

**Ordering (owner, 2026-09-07; discharged 2026-09-08).** This item is Release 0's `L-1` and the kernel's to run. It was held until BL-082 merged, which it now has — v26 is on `master`, so the check below is written once, against the .NET arm. Its third deliverable is an audit check, and BL-082's T-13 replaces the arm every check executes in; started now, the check is written twice. Track E is occupied until the substrate lands.
