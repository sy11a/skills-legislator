...
**Status: IN PROGRESS 2026-08-30 under dev-flow — 13 of 14 tasks owned (T-01…T-13, the last of them with ten appended residue items T-13.1…T-13.10); T-14 (docs, benchmark, converge) is open. The parking condition is met and the owner is executing.** It was PARKED 2026-08-29 for two stated reasons — to road-test dev-flow on a real case, and to keep ownership of the codebase: the first compiled code in this repository is written under the owner's own flow, not by an agent running the plan unattended. That still holds — the case runs one plan task per session, each closed by an owner review; no session starts a task on its own. Branch `bl/082-dotnet-deterministic-substrate`, case `docs/cases/BL-082-dotnet-deterministic-substrate/` (tier 2, feature, R-8201–R-8217, ADR-0008), flow tracker in the operator's knowledge base. Behavioral: `skill/` law text changes command names, a VERSION bump shared with BL-077, full e2e; plus the first `src/` and `tests/` trees and their CI. **The edition number is assigned at merge, never reserved** (roadmap ruling at the v25 merge): the tool pin `src/Legislator.Cli/Version.props` stayed `0.0.0` until Task 12 read the free number — **26** — and the static check has bound `skill/VERSION` to its major since that commit. Rebased onto `master` 2026-09-07 so the benchmark measures the edition that merges; the two backlog rows this branch filed as BL-088/BL-089 moved to BL-091/BL-092, `master` having minted those keys for the Release-0 pair.
